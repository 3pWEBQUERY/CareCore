      ...
      {bodyEditor && typeof document !== "undefined" && createPortal(<div className="body-observation-editor-overlay" role="presentation" onMouseDown={(event) => event.currentTarget === event.target && setBodyEditor(null)}>
        <form className="body-observation-editor" onSubmit={saveBodyObservation} role="dialog" aria-modal="true" aria-labelledby="body-observation-editor-title">
          ...
          <div className="body-observation-editor-fields">
            <label>Art<CareSelect menuZIndex={160} label="Art" value={{ wound: "Wunde", redness: "Rötung", fracture: "Fraktur", other: "Sonstiges" }[bodyEditor.draft.kind]} options={["Wunde", "Rötung", "Fraktur", "Sonstiges"]} onChange={(value) => setBodyEditor((current) => current && ({ ...current, draft: { ...current.draft, kind: ({ Wunde: "wound", Rötung: "redness", Fraktur: "fracture", Sonstiges: "other" } as Record<string, BodyObservation["kind"]>)[value] } }))}/></label>
            <label>Bezeichnung<input required maxLength={120} value={bodyEditor.draft.label} onChange={(event) => setBodyEditor((current) => current && ({ ...current, draft: { ...current.draft, label: event.target.value } }))} placeholder="z. B. Druckstelle"/></label>
            <label>Körperstelle<input required maxLength={160} value={bodyEditor.draft.location} onChange={(event) => setBodyEditor((current) => current && ({ ...current, draft: { ...current.draft, location: event.target.value } }))} placeholder="z. B. rechter Unterarm"/></label>
            <label>Status<CareSelect menuZIndex={160} label="Status" value={bodyEditor.draft.status} options={["Beobachten", "In Behandlung", "Kontrolle geplant", "Abgeschlossen"]} onChange={(value) => setBodyEditor((current) => current && ({ ...current, draft: { ...current.draft, status: value } }))}/></label>
            <label className="body-observation-editor-wide">Beobachtung<textarea rows={3} maxLength={4000} value={bodyEditor.draft.notes} onChange={(event) => setBodyEditor((current) => current && ({ ...current, draft: { ...current.draft, notes: event.target.value } }))} placeholder="Befund, Versorgung und nächste Kontrolle…"/></label>
          </div>
