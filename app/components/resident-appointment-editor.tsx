...
import {
  appointmentCategories, careUnitTaskCategories, draftFromAppointment, initialAppointmentDraft, zurichTimeToIso,
  type AppointmentCareUnit, type AppointmentDraft, type AppointmentKind, type AppointmentResident, type ResidentAppointment,
...
export default function ResidentAppointmentEditor({
  appointment, residents, careUnits = [], residentId, initialDraft, onClose, onSaved,
}: {
  ...
  const changeKind = (kind: AppointmentKind) => setDraft((current) => ({ ...current, kind, residentId: kind === "resident" ? current.residentId : "", careUnitId: kind === "care_unit_task" ? current.careUnitId : "", category: kind === "resident" ? appointmentCategories[0] : careUnitTaskCategories[0] }));
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    if (draft.kind === "resident" && !draft.residentId) { setError("Bitte einen Bewohner auswählen."); return; }
    if (draft.kind === "care_unit_task" && !draft.careUnitId) { setError("Bitte einen Wohnbereich auswählen."); return; }
    ...
    try {
      const response = await fetch(appointment ? `/api/appointments/${appointment.id}` : "/api/appointments", {
        ...
        body: JSON.stringify({ kind: draft.kind, residentId: draft.residentId, careUnitId: draft.careUnitId, title: draft.title, category: draft.category, startsAt, endsAt, location: draft.location, notes: draft.notes, status: draft.status }),
      });
  ...
  return <div className="area-editor-overlay" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target && !saving) onClose(); }}>
    <section className="area-editor-panel appointment-editor-panel" role="dialog" aria-modal="true" aria-labelledby="appointment-editor-title">
      <header className="area-editor-header"><div><p className="eyebrow">CareCore · Betriebskalender</p><h2 id="appointment-editor-title">{appointment ? "Termin verwalten" : "Termin erstellen"}</h2><p>Bewohnertermine erscheinen auch in der Bewohnerakte. Aufgaben für einen Wohnbereich bleiben im Betriebskalender.</p></div><button className="area-editor-close" type="button" aria-label="Terminfenster schliessen" onClick={onClose}><X/></button></header>
      <form className="area-editor-form" onSubmit={save}>
        <div className="area-editor-intro"><span className="area-editor-icon">{draft.kind === "resident" ? <CalendarDots/> : <ClipboardText/>}</span><div><strong>{appointment ? appointment.title : draft.kind === "resident" ? "Neuer Bewohnertermin" : "Neue Wohnbereichsaufgabe"}</strong><p>Alle Zeiten werden für das Alterszentrum in der Zeitzone Zürich geführt.</p></div></div>
        {!residentId && <div className="appointment-kind-switch" role="group" aria-label="Terminart"><button type="button" className={draft.kind === "resident" ? "active" : ""} aria-pressed={draft.kind === "resident"} onClick={() => changeKind("resident")}><CalendarDots/> Bewohnertermin</button><button type="button" className={draft.kind === "care_unit_task" ? "active" : ""} aria-pressed={draft.kind === "care_unit_task"} onClick={() => changeKind("care_unit_task")}><ClipboardText/> Wohnbereichsaufgabe</button></div>}
        <div className="area-editor-grid appointment-editor-grid">
          <label className="area-editor-wide"><span>Bezeichnung</span><input required maxLength={180} value={draft.title} onChange={(event) => update("title", event.target.value)} placeholder={draft.kind === "resident" ? "z. B. Arztvisite, Physiotherapie oder Untersuchung" : "z. B. Materialbestand prüfen oder Übergabe vorbereiten"}/></label>
          {draft.kind === "resident" ? <label><span>Bewohner</span>{residentId ? <input value={selectedResident?.name ?? appointment?.resident_name ?? "Bewohner"} readOnly/> : <CareSelect label="Bewohner" value={selectedResident?.label ?? "Bewohner auswählen"} options={residentOptions.map((item) => item.label)} onChange={(value) => update("residentId", residentOptions.find((item) => item.label === value)?.id ?? "")}/>}</label> : <label><span>Wohnbereich</span><CareSelect label="Wohnbereich" value={selectedCareUnit?.name ?? "Wohnbereich auswählen"} options={careUnits.map((item) => item.name)} onChange={(value) => update("careUnitId", careUnits.find((item) => item.name === value)?.id ?? "")}/></label>}
          <label><span>Kategorie</span><CareSelect label="Kategorie" value={draft.category} options={draft.kind === "resident" ? [...appointmentCategories] : [...careUnitTaskCategories]} onChange={(value) => update("category", value)}/></label>
          <label><span>Datum</span><CareDatePicker label="Termindatum" value={draft.date} onChange={(value) => update("date", value)}/></label>
          <label><span>Ort</span><input maxLength={180} value={draft.location} onChange={(event) => update("location", event.target.value)} placeholder={draft.kind === "resident" ? "z. B. Praxis, Therapieraum oder Spital" : "z. B. Stationszimmer oder Lager"}/></label>
        ...
        {confirmDelete && <div className="appointment-delete-confirm" role="alert"><strong>Termin endgültig löschen?</strong><p>{draft.kind === "resident" ? "Der Eintrag verschwindet auch aus der Bewohnerakte. " : "Der Eintrag verschwindet aus dem Betriebskalender. "}Für eine Absage kannst du stattdessen den Status „Abgesagt“ wählen.</p><div><button className="secondary-button" type="button" onClick={() => setConfirmDelete(false)}>Behalten</button><button className="appointment-danger-button" type="button" disabled={saving} onClick={() => void remove()}>Ja, löschen</button></div></div>}
        <footer className="area-editor-actions appointment-editor-actions">{appointment && !confirmDelete && <button className="appointment-delete-button" type="button" onClick={() => setConfirmDelete(true)}><Trash/> Löschen</button>}<button className="secondary-button" type="button" onClick={onClose}>Abbrechen</button><button className="primary-button" type="submit" disabled={saving || (draft.kind === "resident" ? !residents.length : !careUnits.length)}><Check/> {saving ? "Speichern…" : appointment ? "Änderungen speichern" : "Termin erstellen"}</button></footer>
      </form>
    </section>
  </div>;
}
