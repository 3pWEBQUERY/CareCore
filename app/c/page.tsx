...
export default function Home() {
  ...
  useEffect(() => {
    const onKey = (event: KeyboardEvent) => {
      ...
      if (event.key === "Escape") { setSearchOpen(false); setNoteEditor(null); setViewingNote(null); }
    };
  ...
  const dashboardContent: Record<DashboardWidgetId, ReactNode> = {
    ...
    critical: criticalChange ? <section className="critical-alert" aria-label="Kritischer Hinweis"><span className="critical-symbol"><Icon name="alert"/></span><div><strong>Unmittelbar prüfen · {criticalChange.name}</strong><p>{criticalChange.note}</p></div><button className="secondary-button" type="button" onClick={() => router.push("/c/pflegedokumentation/verlauf")}>Verlauf öffnen <Icon name="chevron" className="button-icon"/></button></section> : null,
  ...
  return <div className="app-shell">
    ...
    {viewingNote && <div className="area-editor-overlay home-note-overlay" role="presentation" onMouseDown={(event) => event.currentTarget === event.target && setViewingNote(null)}><section className="area-editor-panel home-note-panel" role="dialog" aria-modal="true" aria-labelledby="home-note-view-title"><div className="area-editor-header"><div><p className="eyebrow">MEIN ARBEITSPLATZ · PRIVAT</p><h2 id="home-note-view-title">{viewingNote.title}</h2><p>{viewingNote.pinned ? "Angeheftete Notiz" : "Persönliche Notiz"} · Aktualisiert am {new Date(viewingNote.updated_at).toLocaleDateString("de-CH", { day: "numeric", month: "long", year: "numeric" })}</p></div><button className="home-note-close" type="button" aria-label="Schliessen" onClick={() => setViewingNote(null)}><Icon name="close"/></button></div><div className="home-note-reader"><p>{viewingNote.body}</p><div className="home-note-actions"><button className="secondary-button" type="button" onClick={() => setViewingNote(null)}>Schliessen</button><button className="primary-button" type="button" onClick={() => { openNote(viewingNote); setViewingNote(null); }}><Icon name="settings"/> Bearbeiten</button></div></div></section></div>}
