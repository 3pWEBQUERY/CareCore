...
const residents: ResidentVitals[] = [
  {
    ...
    glucose: "6,9",
    weight: "78,4",
    alert: "Systolischer Blutdruck leicht über dem persönlichen Zielbereich.",
  },
  {
    id: "mk",
    initials: "MK",
    ...
    glucose: "8,7",
    weight: "71,2",
    alert: "Blutzucker über dem Zielbereich. Kontrollmessung um 09:30 Uhr geplant.",
  },
  {
    id: "em",
    initials: "EM",
    ...
    glucose: "5,8",
    weight: "65,7",
    alert: "Alle erfassten Werte liegen im persönlichen Zielbereich.",
  },
  {
    id: "rb",
    initials: "RB",
  ...
  },
  {
    ...
    room: "Zimmer 115",
    unit: "Wohnbereich 1",
    measuredAt: "Heute, 07:51",
    status: "Beobachten",
    tone: "attention",
    bloodPressure: "105/66",
    pulse: "88",
    temperature: "37,3",
    oxygen: "94",
    glucose: "7,2",
    weight: "73,0",
    alert: "Sauerstoffsättigung beobachten und nach Mobilisation erneut messen.",
  },
  {
    id: "as",
    initials: "AS",
  ...
  },
  {
    ...
    glucose: "7,8",
    weight: "81,4",
    alert: "Blutdruck und Sauerstoffsättigung auffällig. Pflegefachperson wurde informiert.",
  },
  {
    id: "bk",
    initials: "BK",
    ...
    glucose: "6,5",
    weight: "69,6",
    alert: "Werte stabil, Flüssigkeitszufuhr im Frühdienst weiter beobachten.",
  },
];
...
function trendFor(resident: ResidentVitals, metric: VitalMetric) {
  const base =
    metric === "Blutdruck"
      ? Number(resident.bloodPressure.split("/")[0])
      : metric === "Puls"
        ? Number(resident.pulse)
        : metric === "Temperatur"
          ? Number(resident.temperature.replace(",", "."))
          : metric === "Sauerstoff"
            ? Number(resident.oxygen)
            : Number(resident.glucose.replace(",", "."));
  const offsets =
    metric === "Temperatur" ? [-0.3, -0.1, -0.2, 0] : metric === "Sauerstoff" ? [2, 1, 1, 0] : [-5, -2, -3, 0];
...
export default function VitalsOverviewPage() {
  ...
  return (
    <ModulePageShell
      activeModule="vitals"
      activeChild="Übersicht"
      pageClass="vitals-page"
      locationSecondary="Gesamtes Haus · alle Wohnbereiche"
    >
      {(showToast) => (
        <main className="workspace module-workspace">
          <section className="page-heading care-page-heading" aria-labelledby="vitals-title">
            <div className="heading-copy">
              <p className="eyebrow">CareCore Vitalwerte</p>
              <h1 id="vitals-title">Vitalwerte im Überblick</h1>
              <p>Aktuelle Messungen aller Bewohner, Auffälligkeiten und fällige Kontrollen an einem Ort.</p>
            </div>
            <button
              className="primary-button"
              type="button"
              onClick={() => showToast("Neue Vitalwerterfassung vorbereitet")}
            >
          ...
          <section className="wound-summary" aria-label="Vitalwertstatus im Haus">
            <div>
              ...
              <span>
                <strong>48</strong>
                <small>Bewohner überwacht</small>
              </span>
            </div>
            <div>
              <span className="summary-icon">
            ...
            <div>
              ...
              <span>
                <strong>3</strong>
                <small>Kontrollen fällig</small>
              </span>
            </div>
          </section>

          <section className="critical-alert vitals-alert" aria-label="Klinischer Hinweis">
            <span className="critical-symbol">
              <ModuleIcon name="alert" />
            </span>
            <div>
              <strong>Kontrollmessung erforderlich · Maria Keller</strong>
              <p>Blutzucker 8,7 mmol/l. Nächste Messung ist um 09:30 Uhr vorgesehen.</p>
            </div>
            <button
              ...
              onClick={() => {
                setSelectedId("mk");
                setMetric("Blutzucker");
                showToast("Maria Keller ausgewählt");
              }}
            >
              Messverlauf ansehen <ModuleIcon name="chevron" className="button-icon" />
            </button>
          </section>
          ...
          <div className="vitals-layout">
            <section className="card vitals-browser" aria-labelledby="vitals-list-title">
              <div className="vitals-toolbar">
                <div>
                  ...
                  <p className="card-subtitle">
                    {filteredResidents.length} von {residents.length} Bewohnern in dieser Demo
                  </p>
                </div>
                ...
                <label className="vitals-unit-filter">
                  ...
                  <select value={unit} onChange={(event) => setUnit(event.target.value)}>
                    <option>Gesamtes Haus</option>
                    <option>Wohnbereich 1</option>
                    <option>Wohnbereich 2</option>
                    <option>Wohnbereich 3</option>
                    <option>Pflegewohngruppe</option>
                  </select>
                </label>
              ...
              <div className="vitals-list">
                {filteredResidents.map((resident) => (
                  <button
                    className={`vitals-row ${selectedResident.id === resident.id ? "selected" : ""}`}
                    type="button"
                    key={resident.id}
                  ...
                  >
                    ...
                    <span className="vitals-person">
                      ...
                      <em>{resident.measuredAt}</em>
                    </span>
                    <span className="vitals-values">
                      <span>
                        <small>BD</small>
                        <strong>{resident.bloodPressure}</strong>
                        <em>mmHg</em>
                      </span>
                      <span>
                        <small>Puls</small>
                        <strong>{resident.pulse}</strong>
                        <em>/min</em>
                      </span>
                      <span>
                        <small>Temp.</small>
                        <strong>{resident.temperature}</strong>
                        <em>°C</em>
                      </span>
                      <span>
                        <small>SpO₂</small>
                        <strong>{resident.oxygen}</strong>
                        <em>%</em>
                      </span>
                      <span>
                        <small>BZ</small>
                        <strong>{resident.glucose}</strong>
                        <em>mmol/l</em>
                      </span>
                      <span>
                        <small>Gewicht</small>
                        <strong>{resident.weight}</strong>
                        <em>kg</em>
                      </span>
                    </span>
                ...
                {filteredResidents.length === 0 && (
                  <div className="resident-empty">
                    <ModuleIcon name="search" />
                    <strong>Keine Vitalwerte gefunden</strong>
            ...
            <aside className="vitals-detail" aria-live="polite">
              <section className="card vitals-detail-card">
                ...
                <div className={`vitals-clinical-note ${selectedResident.tone}`}>
                  <ModuleIcon name={selectedResident.tone === "stable" ? "check" : "alert"} />
                  <p>
                    <strong>Klinische Einordnung</strong>
                    <span>{selectedResident.alert}</span>
                  </p>
                </div>
                <div className="vitals-detail-actions">
                  <button
                    className="primary-button"
                    type="button"
                    onClick={() => showToast(`Vitalwerterfassung für ${selectedResident.name} vorbereitet`)}
                  >
                    Neue Messung
                  </button>
                  <button
                    className="secondary-button"
                    type="button"
                    onClick={() => showToast(`Bewohnerakte von ${selectedResident.name} geöffnet`)}
                  >
                    Bewohnerakte
                  </button>
                </div>
              ...
              <section className="card vitals-trend-card">
                <div className="card-header">
                  <div>
                    ...
                    <h2 className="card-title">Letzte 24 Stunden</h2>
                  </div>
                </div>
                ...
                <div className="vitals-chart" aria-label={`${metric}-Verlauf von ${selectedResident.name}`}>
                  {trend.map((point) => {
                    const height = 38 + ((point.value - minimum) / Math.max(maximum - minimum, 1)) * 48;
                    return (
                      <div className="vitals-chart-point" key={point.label}>
                        <strong>{String(point.value).replace(".", ",")}</strong>
                        <span>
                          <i style={{ height: `${height}%` }} />
                        </span>
                        <small>{point.label.replace("Gestern ", "G · ").replace("Heute ", "H · ")}</small>
                      </div>
                    );
                  })}
                </div>
                <p className="vitals-chart-note">
                  <ModuleIcon name="vitals" />
                  Persönliche Grenzwerte werden bei jeder Messung automatisch geprüft.
                </p>
              </section>
              ...
              <section className="card vitals-due-card">
                <div className="card-header">
                  <div>
                    ...
                    <h2 className="card-title">Nächste Kontrollen</h2>
                  </div>
                </div>
                <div>
                  <button type="button" onClick={() => setSelectedId("mk")}>
                    <time>09:30</time>
                    <span>
                      <strong>Maria Keller</strong>
                      <small>Blutzucker-Kontrolle</small>
                    </span>
                    <ModuleIcon name="chevron" />
                  </button>
                  <button type="button" onClick={() => setSelectedId("pa")}>
                    <time>10:00</time>
                    <span>
                      <strong>Peter Aebischer</strong>
                      <small>SpO₂ nach Mobilisation</small>
                    </span>
                    <ModuleIcon name="chevron" />
                  </button>
                  <button type="button" onClick={() => setSelectedId("wb")}>
                    <time>10:15</time>
                    <span>
                      <strong>Walter Brunner</strong>
                      <small>Blutdruck &amp; SpO₂</small>
                    </span>
                    <ModuleIcon name="chevron" />
                  </button>
                </div>
