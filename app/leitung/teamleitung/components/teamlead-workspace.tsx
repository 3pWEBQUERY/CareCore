...
import { Archive, ArrowClockwise, CalendarDots, CheckCircle, ClipboardText, Clock, DotsThree, PencilSimple, Plus, UserPlus, UsersThree, WarningCircle } from "@phosphor-icons/react";
...
type Employee = { id: string; display_name: string; username?: string; role: string; active: boolean; archived_at?: string | null; care_unit_name?: string; primary_care_unit_id?: string | null; job_title?: string; phone?: string };
...
export default function TeamleadWorkspace({ view }: { view: View }) {
  ...
  return <ModulePageShell activeModule="teamlead" activeChild={content.title} pageClass="leadership-page">{() => <main className="workspace teamlead-workspace">
    ...
    {!canLead ? <section className="teamlead-denied"><WarningCircle/><div><strong>Zugriff beschränkt</strong><p>Dieser Bereich ist ausschließlich für Administratoren und Leitungen verfügbar.</p></div></section> : <>
      ...
      {view === "employees" && <EmployeesView rows={rows} isArchived={isArchived} onUpdate={updateState} onSelect={setSelectedEmployee}/>}
    ...
    {selectedEmployee && <TeamleadEmployeeEditor employee={selectedEmployee} units={units} onClose={() => setSelectedEmployee(null)} onUpdated={async (message) => { setSelectedEmployee(null); setNotice(message); await load(); }}/>}
  </main>}</ModulePageShell>;
}

function EmployeesView({ rows, isArchived, onUpdate, onSelect }: { rows: TeamleadRow[]; isArchived: (row: TeamleadRow) => boolean; onUpdate: (id: string, action: "archive" | "restore") => void; onSelect: (employee: TeamleadRow) => void }) {
  return <section className="teamlead-grid teamlead-employees-grid"><article className="teamlead-panel"><div className="teamlead-panel-head"><div><p className="eyebrow">TEAMVERZEICHNIS</p><h2>Mitarbeitende & Rollen</h2><span>{rows.length} Profile im Überblick</span></div><button type="button" className="teamlead-icon-button" aria-label="Weitere Optionen"><DotsThree/></button></div><div className="teamlead-table teamlead-employee-table"><div className="teamlead-table-head"><span>Mitarbeiter</span><span>Rolle</span><span>Arbeitsplatz</span><span>Status</span><span>Aktionen</span></div>{rows.map((row) => <div className="teamlead-table-row teamlead-employee-row" role="button" tabIndex={0} key={row.id} onClick={() => onSelect(row)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onSelect(row); } }}><div className="teamlead-person"><b>{initials(row.display_name)}</b><span><strong>{row.display_name}</strong><small>@{row.username}</small></span></div><span className="teamlead-role">{labelRole(row.role)}</span><span>{row.care_unit_name || "Kein fester Bereich"}</span><span className={`teamlead-status ${isArchived(row) ? "archived" : "active"}`}><i/>{isArchived(row) ? "Archiviert" : "Aktiv"}</span><span className="teamlead-row-actions"><button className="teamlead-row-icon" type="button" aria-label={`${row.display_name} bearbeiten`} onClick={(event) => { event.stopPropagation(); onSelect(row); }}><PencilSimple/><SidebarTooltip label="Mitarbeiter bearbeiten"/></button><button className="teamlead-row-icon" type="button" aria-label={isArchived(row) ? `${row.display_name} wiederherstellen` : `${row.display_name} archivieren`} onClick={(event) => { event.stopPropagation(); onUpdate(row.id, isArchived(row) ? "restore" : "archive"); }}>{isArchived(row) ? <ArrowClockwise/> : <Archive/>}<SidebarTooltip label={isArchived(row) ? "Mitarbeiter wiederherstellen" : "Mitarbeiter archivieren"}/></button></span></div>)}</div></article><aside className="teamlead-aside"><article className="teamlead-panel teamlead-role-card"><p className="eyebrow">ROLLEN & ZUGRIFFE</p><h2>Verteilung im Team</h2><div className="teamlead-role-breakdown">{["leitung", "pflege", "arzt", "mitarbeitende:r"].map((role) => <div key={role}><span><i className={`role-dot ${role}`}/>{labelRole(role)}</span><strong>{rows.filter((row) => row.role === role && !isArchived(row)).length}</strong></div>)}</div></article><article className="teamlead-info-card"><UsersThree/><div><strong>Team aktuell halten</strong><p>Klicke auf ein Profil, um Rolle, Arbeitsbereich und Archivstatus zu verwalten.</p></div></article></aside></section>;
}
...
function TeamleadEmployeeEditor({ employee, units, onClose, onUpdated }: { employee: TeamleadRow; units: Unit[]; onClose: () => void; onUpdated: (message: string) => Promise<void> }) {
  ...
  const roleKey = (value: string) => ({ "Mitarbeitende:r": "mitarbeitende:r", Pflege: "pflege", "Ärztlicher Dienst": "arzt", Leitung: "leitung" }[value] ?? "mitarbeitende:r");

  async function request(method: "PATCH" | "DELETE", payload: Record<string, string | null>) {
    setSaving(true); setError("");
    try {
      const response = await fetch("/api/teamlead/employees", { method, headers: { "content-type": "application/json" }, body: JSON.stringify({ id: employee.id, ...payload }) });
      const data = await response.json().catch(() => ({})) as { error?: string };
      if (!response.ok) throw new Error(data.error ?? "Die Änderung konnte nicht gespeichert werden.");
      await onUpdated(method === "DELETE" ? "Mitarbeiter wurde endgültig gelöscht." : payload.action === "archive" ? "Mitarbeiter wurde archiviert." : payload.action === "restore" ? "Mitarbeiter wurde wiederhergestellt." : "Mitarbeiterprofil wurde gespeichert.");
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Die Änderung konnte nicht gespeichert werden."); }
    finally { setSaving(false); }
  }

  return <div className="user-editor-overlay" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) onClose(); }}><section className="user-editor-panel" role="dialog" aria-modal="true" aria-labelledby="teamlead-employee-title"><header className="user-editor-header"><div><p className="eyebrow">CareCore Teamleitung · Mitarbeitende</p><h2 id="teamlead-employee-title">{employee.display_name} verwalten</h2><p>Profil, Rolle und Arbeitsbereich werden nachvollziehbar in Neon gespeichert.</p></div><button className="profile-panel-close" type="button" onClick={onClose} aria-label="Mitarbeiterverwaltung schliessen">×</button></header><div className="user-editor-body"><aside className="user-editor-summary"><span className="profile-large-avatar">{initials(displayName || employee.display_name)}</span><h3>{displayName || employee.display_name}</h3><p>@{username || employee.username}</p><div className="user-editor-actions"><button className={mode === "edit" ? "active" : ""} type="button" onClick={() => setMode("edit")}>Profil bearbeiten</button><button className={mode === "archive" ? "active warning" : "warning"} type="button" onClick={() => setMode("archive")}>{employee.active ? "Sperren & archivieren" : "Wiederherstellen"}</button><button className={mode === "delete" ? "active danger" : "danger"} type="button" onClick={() => setMode("delete")}>Endgültig löschen</button></div></aside><section className="user-editor-content">{mode === "edit" ? <form onSubmit={(event) => { event.preventDefault(); void request("PATCH", { displayName, username, role, jobTitle, phone, primaryCareUnitId: unitId || null }); }}><div className="user-editor-section-heading"><p className="eyebrow">Profil und Berechtigungen</p><h3>Mitarbeiterprofil</h3><p>Aktualisiere die Angaben für die tägliche Zusammenarbeit im Team.</p></div><div className="user-editor-fields"><label>Name<input required value={displayName} onChange={(event) => setDisplayName(event.target.value)}/></label><label>Benutzername<input required value={username} onChange={(event) => setUsername(event.target.value.replace(/\s/g, ""))}/></label><label>Funktion<input value={jobTitle} placeholder="z. B. Pflegefachperson HF" onChange={(event) => setJobTitle(event.target.value)}/></label><label>Telefon<input value={phone} placeholder="z. B. +41 79 123 45 67" onChange={(event) => setPhone(event.target.value)}/></label><label>Rolle<CareSelect label="Rolle auswählen" value={labelRole(role)} options={roleOptions} onChange={(value) => setRole(roleKey(value))}/></label><label>Fester Arbeitsbereich<CareSelect label="Festen Arbeitsbereich auswählen" value={unitName} options={["Nicht festgelegt", ...units.map((unit) => unit.name)]} onChange={(name) => setUnitId(units.find((unit) => unit.name === name)?.id ?? "")}/></label></div>{error && <p className="user-editor-error">{error}</p>}<footer className="user-editor-footer"><button className="primary-button" disabled={saving}>Änderungen speichern</button></footer></form> : <div className="user-editor-confirm"><p className="eyebrow">{mode === "delete" ? "Irreversible Aktion" : "Kontostatus"}</p><h3>{mode === "delete" ? "Mitarbeiter endgültig löschen" : employee.active ? "Mitarbeiter archivieren" : "Mitarbeiter wiederherstellen"}</h3><p>{mode === "delete" ? "Das Mitarbeiterkonto wird inklusive Zugang dauerhaft entfernt." : employee.active ? "Das Profil wird aus dem Tagesgeschäft entfernt und als Archiv geführt." : "Das Profil wird wieder aktiviert und steht dem Team wieder zur Verfügung."}</p>{error && <p className="user-editor-error">{error}</p>}<div className="user-editor-footer"><button className="secondary-button" type="button" onClick={() => setMode("edit")}>Abbrechen</button><button className={mode === "delete" || employee.active ? "danger-button" : "primary-button"} type="button" disabled={saving} onClick={() => void request(mode === "delete" ? "DELETE" : "PATCH", mode === "delete" ? {} : { action: employee.active ? "archive" : "restore" })}>{mode === "delete" ? "Endgültig löschen" : employee.active ? "Archivieren" : "Wiederherstellen"}</button></div></div>}</section></div></section></div>;
}
...
function TeamleadForm({ view, form, setForm, units, employees, onClose, onSubmit }: { view: View; form: Record<string, string>; setForm: (value: Record<string, string>) => void; units: Unit[]; employees: Employee[]; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
  ...
  const intro = view === "employees"
    ...
    : view === "shifts"
      ? { title: "Neuen Dienst planen", copy: "Plane Einsatzzeit, Wohnbereich und Besetzung direkt im Teamkontext.", status: "Bereit zum Planen" }
      : { title: "Neue Teamaufgabe", copy: "Halte Verantwortlichkeit, Priorität und Termin nachvollziehbar fest.", status: "Bereit zum Erstellen" };
  const summary = view === "employees"
    ? { primary: form.displayName || "Neues Mitarbeiterprofil", secondary: `${labelRole(form.role)} · ${form.username ? `@${form.username}` : "Benutzername folgt"}`, context: careUnitName || "Noch kein fester Wohnbereich", contextLabel: "Arbeitsbereich" }
    : view === "shifts"
      ? { primary: form.name || "Neuer Dienst", secondary: form.startsAt ? `Beginn ${formatDate(form.startsAt)}` : "Zeit noch festlegen", context: assignedEmployee || "Noch nicht zugewiesen", contextLabel: "Besetzung" }
      : { primary: form.title || "Neue Aufgabe", secondary: priorityLabel(form.priority), context: assignedEmployee || "Noch nicht zugewiesen", contextLabel: "Verantwortung" };

  return <div className="area-editor-overlay teamlead-editor-overlay" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) onClose(); }}>
    <section className="area-editor-panel teamlead-editor" role="dialog" aria-modal="true" aria-labelledby="teamlead-editor-title">
      <header className="area-editor-header">
        <div><p className="eyebrow">CARECORE TEAMLEITUNG</p><h2 id="teamlead-editor-title">{label}</h2><p>Die Angaben können später jederzeit angepasst werden.</p></div>
        <button className="area-editor-close" type="button" onClick={onClose} aria-label="Editor schliessen">×</button>
      </header>
      <form id="teamlead-form" className="area-editor-form" onSubmit={onSubmit}>
        <div className="area-editor-intro">
          <span className="area-editor-icon">{view === "employees" ? <UsersThree/> : view === "shifts" ? <CalendarDots/> : <ClipboardText/>}</span>
          <div><strong>{intro.title}</strong><p>{intro.copy}</p></div>
          <span className="duty-assignment-status"><i/>{intro.status}</span>
        </div>
        <div className="area-editor-grid">
          ...
          {view === "shifts" && <>
            ...
            <label>Wohnbereich<select value={form.careUnitId ?? ""} onChange={(event) => update("careUnitId", event.target.value)}><option value="">Bereich übergreifend</option>{units.map((unit) => <option key={unit.id} value={unit.id}>{unit.name}</option>)}</select></label>
            <label>Beginn<input required type="datetime-local" value={form.startsAt ?? ""} onChange={(event) => update("startsAt", event.target.value)}/></label>
            <label>Ende<input required type="datetime-local" value={form.endsAt ?? ""} onChange={(event) => update("endsAt", event.target.value)}/></label>
            <label className="area-editor-wide">Mitarbeiter zuweisen<select value={form.employeeId ?? ""} onChange={(event) => update("employeeId", event.target.value)}><option value="">Noch nicht zuweisen</option>{employees.map((employee) => <option key={employee.id} value={employee.id}>{employee.display_name}</option>)}</select></label>
          </>}
          {view === "tasks" && <>
            <label className="area-editor-wide">Titel<input required placeholder="Was soll erledigt werden?" value={form.title ?? ""} onChange={(event) => update("title", event.target.value)}/></label>
            <label className="area-editor-wide">Beschreibung<textarea placeholder="Kontext, gewünschtes Ergebnis oder Hinweise" value={form.description ?? ""} onChange={(event) => update("description", event.target.value)}/></label>
            <label>Priorität<select value={form.priority ?? "normal"} onChange={(event) => update("priority", event.target.value)}><option value="low">Niedrig</option><option value="normal">Normal</option><option value="high">Hoch</option><option value="critical">Kritisch</option></select></label>
            <label>Zuweisen<select value={form.assignedTo ?? ""} onChange={(event) => update("assignedTo", event.target.value)}><option value="">Nicht zugewiesen</option>{employees.map((employee) => <option key={employee.id} value={employee.id}>{employee.display_name}</option>)}</select></label>
            <label className="area-editor-wide">Fällig am<input type="datetime-local" value={form.dueAt ?? ""} onChange={(event) => update("dueAt", event.target.value)}/></label>
          </>}
        </div>
        <div className="duty-assignment-summary teamlead-editor-summary"><span><strong>{summary.primary}</strong><small>{summary.secondary}</small></span><span><strong>{summary.context}</strong><small>{summary.contextLabel}</small></span></div>
        <footer className="area-editor-actions"><button className="secondary-button" type="button" onClick={onClose}>Abbrechen</button><button className="primary-button" type="submit">{label}</button></footer>
      </form>
    </section>
  </div>;
}
