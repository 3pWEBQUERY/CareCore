...
export async function GET() {
  try {
    ...
    console.error("Documentation GET failed", error);
    return NextResponse.json({ error: "Dokumentation konnte nicht geladen werden." }, { status: 500 });
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    const importance = ["standard", "important", "visit", "observation", "critical"].includes(String(body.importance))
    ...
    if (!residentName || !content)
      return NextResponse.json({ error: "Bewohner und Eintrag sind Pflichtfelder." }, { status: 400 });
    const sql = carecoreDb();
    const residents =
      await sql`SELECT r.id, stay.care_unit_id FROM carecore_residents r LEFT JOIN LATERAL (SELECT care_unit_id FROM carecore_resident_stays WHERE resident_id = r.id AND ended_at IS NULL ORDER BY started_at DESC LIMIT 1) stay ON TRUE WHERE r.organization_id = ${actor.organizationId ?? null} AND r.first_name || ' ' || r.last_name = ${residentName} LIMIT 1`;
    ...
    console.error("Documentation POST failed", error);
    return NextResponse.json({ error: "Dokumentation konnte nicht gespeichert werden." }, { status: 500 });
  }
}
