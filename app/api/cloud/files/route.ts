...
export async function GET() {
  try {
    ...
    ` as unknown as Array<{ id: string; name: string; mime_type: string; size_bytes: number; uploaded_by: string | null; created_at: string; updated_at: string }>;
...
export async function POST(request: Request) {
  try {
    ...
    if (!(file instanceof File) || file.size === 0) return NextResponse.json({ error: "Bitte wähle eine Datei aus." }, { status: 400 });
    if (file.size > MAX_FILE_BYTES) return NextResponse.json({ error: "Dateien dürfen höchstens 4 MB gross sein." }, { status: 413 });
    const name = file.name.trim().replace(/[\\/\u0000-\u001f]/g, "-").slice(0, 220);
