...
export async function POST(request: Request, context: { params: Promise<{ residentId: string }> }) {
  try {
    ...
    if (!kinds.includes(kind) || !label || label.length > 120 || !location || location.length > 160 || status.length > 120 || notes.length > 4000 || ![x,y,z].every((value) => Number.isFinite(value) && Math.abs(value) <= 3)) return NextResponse.json({ error: "Bitte Befund und Körperstelle vollständig angeben." }, { status: 400 });
