...
export async function GET(request: Request, context: { params: Promise<{ residentId: string }> }) {
  try {
    ...
    const photoDataUrl = resident?.photo_base64 && allowedTypes.has(String(resident.photo_mime_type))
      ...
      : null;
    return NextResponse.json({ photoDataUrl, updatedAt: resident?.photo_updated_at ?? null }, { headers: { "Cache-Control": "no-store" } });
  } catch (error) {
...
export async function PUT(request: Request, context: { params: Promise<{ residentId: string }> }) {
  try {
    ...
    if (!match || !allowedTypes.has(match[1])) return NextResponse.json({ error: "Erlaubt sind JPEG-, PNG- und WebP-Bilder." }, { status: 400 });
    const image = Buffer.from(match[2], "base64");
    if (!image.length || image.length > MAX_PHOTO_BYTES) return NextResponse.json({ error: "Das optimierte Bild darf höchstens 1 MB gross sein." }, { status: 413 });
    if (!hasValidImageSignature(image, match[1])) return NextResponse.json({ error: "Die Bilddatei ist ungültig." }, { status: 400 });
    const base64 = image.toString("base64");
    const rows = await sql`UPDATE carecore_residents SET photo_base64 = ${base64}, photo_mime_type = ${match[1]}, photo_updated_at = NOW(), updated_at = NOW() WHERE id = ${residentId} RETURNING photo_updated_at`;
    if (!rows[0]) return NextResponse.json({ error: "Bewohnerakte nicht gefunden." }, { status: 404 });
    return NextResponse.json({ photoDataUrl: `data:${match[1]};base64,${base64}`, updatedAt: rows[0].photo_updated_at });
  } catch (error) {
