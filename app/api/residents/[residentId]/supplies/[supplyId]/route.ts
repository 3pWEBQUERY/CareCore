...
export async function PATCH(request: Request, context: { params: Promise<{ residentId: string; supplyId: string }> }) {
  try {
    ...
    const rows =
      await active.sql`UPDATE carecore_resident_supplies SET item_name = ${input.itemName}, category = ${input.category}, unit = ${input.unit}, current_quantity = ${input.currentQuantity}, target_quantity = ${input.targetQuantity}, status = ${input.status}, notes = ${input.notes || null}, updated_by = ${active.actor.id}, updated_at = NOW() WHERE id = ${supplyId} RETURNING id, product_id, item_name, category, unit, current_quantity, target_quantity, status, notes, updated_at`;
