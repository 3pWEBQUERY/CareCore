...
export async function GET() {
  const actor = await carecoreActor(); if (!actor?.organizationId || !hasPermission(actor, "schedule.manage")) return forbidden();
...
export async function POST(request: Request) {
  ...
  if (body.careUnitId) {
    const unit = await sql`SELECT cu.id FROM carecore_care_units cu JOIN carecore_sites si ON si.id = cu.site_id WHERE cu.id = ${body.careUnitId} AND si.organization_id = ${actor.organizationId} LIMIT 1`;
    if (!unit[0]) return NextResponse.json({ error: "Der gewählte Wohnbereich ist nicht verfügbar." }, { status: 400 });
  }
  const employeeIds = body.employeeIds?.length ? (await sql`SELECT user_id FROM carecore_user_profiles WHERE organization_id = ${actor.organizationId} AND user_id = ANY(${body.employeeIds}::uuid[])`).map((row) => row.user_id as string) : [];
  ...
  for (const userId of employeeIds) await sql`INSERT INTO carecore_shift_assignments (id, shift_id, user_id, role) VALUES (${randomUUID()}, ${id}, ${userId}, 'Mitarbeitende:r') ON CONFLICT (shift_id, user_id) DO NOTHING`;
...
export async function PATCH(request: Request) {
  ...
  const sql = carecoreDb(); const rows = await sql`UPDATE carecore_shifts SET status = ${body.action === "archive" ? "cancelled" : "planned"} WHERE id = ${body.id} AND care_unit_id IN (SELECT cu.id FROM carecore_care_units cu JOIN carecore_sites si ON si.id = cu.site_id WHERE si.organization_id = ${actor.organizationId}) RETURNING id`;
