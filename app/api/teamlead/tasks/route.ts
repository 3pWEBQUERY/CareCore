...
export async function POST(request: Request) {
  ...
  const sql = carecoreDb(); const id = randomUUID(); const priority = ["low", "normal", "high", "critical"].includes(body.priority ?? "") ? body.priority : "normal";
...
export async function PATCH(request: Request) {
  ...
  const sql = carecoreDb();
  if (body.action === "archive") await sql`UPDATE carecore_tasks SET status = 'cancelled', updated_at = NOW() WHERE id = ${body.id}`;
  else if (body.action === "restore") await sql`UPDATE carecore_tasks SET status = 'open', updated_at = NOW() WHERE id = ${body.id}`;
  else await sql`UPDATE carecore_tasks SET title = COALESCE(${body.title ?? null}, title), priority = COALESCE(${body.priority ?? null}, priority), assigned_to = COALESCE(${body.assignedTo ?? null}, assigned_to), updated_at = NOW() WHERE id = ${body.id}`;
