...
const permittedActor = (actor: CarecoreActor | null): actor is CarecoreActor & { organizationId: string } => !!actor?.organizationId && hasPermission(actor, "team.manage");
...
export async function GET() {
  ...
  const units = await sql`SELECT cu.id, cu.name FROM carecore_care_units cu JOIN carecore_sites si ON si.id = cu.site_id WHERE si.organization_id = ${actor.organizationId} AND cu.active = TRUE ORDER BY cu.name`;
...
export async function POST(request: Request) {
  ...
  const sql = carecoreDb();
  if (typeof body.primaryCareUnitId === "string" && body.primaryCareUnitId && !(await careUnitInOrganization(sql, body.primaryCareUnitId, actor.organizationId))[0]) return NextResponse.json({ error: "Der gewählte Wohnbereich ist nicht verfügbar." }, { status: 400 });
...
export async function PATCH(request: Request) {
  ...
  const sql = carecoreDb();
  const permitted = await sql`SELECT u.id, u.role FROM carecore_users u INNER JOIN carecore_user_profiles p ON p.user_id = u.id WHERE u.id = ${body.id} AND p.organization_id = ${actor.organizationId} LIMIT 1`;
  if (!permitted[0]) return NextResponse.json({ error: "Mitarbeiter ist in diesem Arbeitsbereich nicht verfügbar." }, { status: 404 });
  if (permitted[0].role === "admin" && !hasPermission(actor, "administration.manage")) return forbidden("Administrationskonten können nur von der Administration bearbeitet werden.");
