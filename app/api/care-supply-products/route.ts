...
type ProductInput = { itemName?: unknown; category?: unknown; unit?: unknown; description?: unknown; defaultTargetQuantity?: unknown; status?: unknown };
...
export async function GET() {
  try {
    ...
    const products = actor.role === "admin"
      ? await sql`SELECT id, item_name, category, unit, description, default_target_quantity, status, created_at, updated_at FROM carecore_care_supply_products WHERE organization_id = ${actor.organizationId} ORDER BY status = 'active' DESC, category, item_name`
      : await sql`SELECT id, item_name, category, unit, description, default_target_quantity, status, created_at, updated_at FROM carecore_care_supply_products WHERE organization_id = ${actor.organizationId} AND status = 'active' ORDER BY category, item_name`;
...
export async function POST(request: Request) {
  try {
    ...
    const sql = carecoreDb();
    const rows = await sql`INSERT INTO carecore_care_supply_products (id, organization_id, item_name, category, unit, description, default_target_quantity, created_by, updated_by) VALUES (${randomUUID()}, ${actor.organizationId}, ${itemName}, ${category}, ${unit}, ${description || null}, ${target}, ${actor.id}, ${actor.id}) RETURNING id, item_name, category, unit, description, default_target_quantity, status, created_at, updated_at`;
...
export async function PATCH(request: Request) {
  try {
    ...
    const sql = carecoreDb();
    const rows = await sql`UPDATE carecore_care_supply_products SET item_name = ${itemName}, category = ${category}, unit = ${unit}, description = ${description || null}, default_target_quantity = ${quantity(input.defaultTargetQuantity)}, status = ${status}, updated_by = ${actor.id}, updated_at = NOW() WHERE id = ${id} AND organization_id = ${actor.organizationId} RETURNING id, item_name, category, unit, description, default_target_quantity, status, created_at, updated_at`;
