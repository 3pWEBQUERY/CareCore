...
export async function POST(request: Request) {
  try {
    ...
    const priority = ["low", "normal", "high", "critical"].includes(String(body.priority))
    ...
    const residents = residentName
      ? await sql`SELECT id FROM carecore_residents WHERE organization_id = ${actor.organizationId} AND first_name || ' ' || last_name = ${residentName} LIMIT 1`
      : [];
    ...
    return NextResponse.json({ error: "Aufgabe konnte nicht gespeichert werden." }, { status: 500 });
  }
}
...
export async function PATCH(request: Request) {
  try {
    ...
    if (typeof body.id !== "string" || !/^[0-9a-f-]{36}$/i.test(body.id) || typeof body.completed !== "boolean")
      return NextResponse.json({ error: "Ungültige Aufgabe." }, { status: 400 });
    const sql = carecoreDb();
    const rows =
      await sql`UPDATE carecore_tasks SET status = ${body.completed ? "completed" : "open"}, completed_at = ${body.completed ? new Date().toISOString() : null}, updated_at = NOW() WHERE id = ${body.id} AND organization_id = ${actor.organizationId ?? null} RETURNING id`;
