...
export async function GET(request: Request) {
  try {
    ...
    const selectedId = conversations.some((conversation) => conversation.id === requestedId)
      ? requestedId!
    ...
    if (body.action === "conversation") {
      ...
      const available = await Promise.all(
        selectedMembers.map(async (userId) => {
          const rows =
            await sql`SELECT u.id FROM carecore_users u INNER JOIN carecore_user_profiles p ON p.user_id = u.id WHERE p.organization_id = ${actor.organizationId} AND u.active = TRUE AND u.id = ${userId} LIMIT 1`;
          return rows[0]?.id;
        }),
      );
      ...
      for (const userId of [actor.id, ...selectedMembers])
        await sql`INSERT INTO carecore_conversation_members (conversation_id, user_id, last_read_at) VALUES (${id}, ${userId}, ${userId === actor.id ? new Date() : null})`;
