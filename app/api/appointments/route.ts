...
export async function GET(request: Request) {
  try {
    ...
    const [appointments, residents] = await Promise.all([
      sql`SELECT a.id, a.resident_id, CONCAT(r.first_name, ' ', r.last_name) AS resident_name, r.status AS resident_status,
        ...
        JOIN carecore_residents r ON r.id = a.resident_id AND r.organization_id = a.organization_id
    ...
    return NextResponse.json({ appointments, residents });
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    ...
    const id = randomUUID();
    await sql`INSERT INTO carecore_resident_appointments (id, organization_id, resident_id, title, category, starts_at, ends_at, location, notes, status, created_by, updated_by)
      VALUES (${id}, ${actor.organizationId}, ${parsed.residentId}, ${parsed.title}, ${parsed.category}, ${parsed.startsAt}, ${parsed.endsAt}, ${parsed.location || null}, ${parsed.notes || null}, ${parsed.status}, ${actor.id}, ${actor.id})`;
