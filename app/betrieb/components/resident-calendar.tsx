...
const dateHeading = (key: string, options: Intl.DateTimeFormatOptions) =>
  new Intl.DateTimeFormat("de-CH", { timeZone: "UTC", ...options }).format(dateFromKey(key));
const statusName = (status: ResidentAppointment["status"]) =>
  status === "completed" ? "Abgeschlossen" : status === "cancelled" ? "Abgesagt" : "Geplant";
const eventTime = (appointment: ResidentAppointment) =>
          ...
          <div className="resident-calendar-layout">
            <aside className="card resident-calendar-sidebar">
              ...
              <div className="resident-calendar-sidebar-section resident-calendar-agenda">
                ...
                {!selectedDay.length && (
                  <p className="resident-calendar-agenda-empty">
                    Wähle einen anderen Tag oder plane einen neuen Termin.
                  </p>
                )}
              </div>
            </aside>
                        ...
                        >
                          ...
                          <div className="resident-calendar-month-events">
                            ...
                            {entries.length > 3 && (
                              <button
                                type="button"
                                className="resident-calendar-more"
                                onClick={() => setFocusDate(date)}
                              >
