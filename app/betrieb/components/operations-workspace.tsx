...
function TaskEditor({ open, onClose, showToast }: { open: boolean; onClose: () => void; showToast: (message: string) => void }) {
...
function ShiftStartEditor({ open, onClose, showToast }: { open: boolean; onClose: () => void; showToast: (message: string) => void }) {
  ...
  const toggleChecklist = (item: string) => setChecklist((current) => current.includes(item) ? current.filter((entry) => entry !== item) : [...current, item]);
  if (!open) return null;
  return <div className="area-editor-overlay" role="presentation" onMouseDown={(event) => event.currentTarget === event.target && onClose()}><section className="area-editor-panel shift-start-panel" role="dialog" aria-modal="true" aria-labelledby="shift-start-title"><header className="area-editor-header"><div><p className="eyebrow">CareCore Shift · Schicht</p><h2 id="shift-start-title">Dienst starten</h2><p>Starte deinen Dienst mit einem kurzen Check-in und halte fest, dass die wichtigsten Übergabepunkte geprüft sind.</p></div><button className="area-editor-close" type="button" onClick={onClose} aria-label="Dienststart schliessen">×</button></header><form className="area-editor-form" onSubmit={(event) => { event.preventDefault(); onClose(); showToast(`${shift} im ${unit} wurde gestartet`); }}><div className="area-editor-intro"><span className="area-editor-icon"><ModuleIcon name="shift"/></span><div><strong>Schicht-Check-in</strong><p>Dein Start wird mit Uhrzeit und Arbeitsbereich im Schichtverlauf gespeichert.</p></div><span className="duty-assignment-status"><i/>Bereit zum Start</span></div><div className="area-editor-grid"><label>Arbeitsbereich<ScheduleSelect label="Arbeitsbereich" value={unit} options={["Wohnbereich 1", "Wohnbereich 2", "Wohnbereich 3", "Gesamtes Haus"]} onChange={setUnit}/></label><label>Dienst<ScheduleSelect label="Dienst" value={shift} options={["Frühdienst", "Spätdienst", "Nachtwache", "Bereitschaft"]} onChange={setShift}/></label><label>Startdatum<ScheduleDatePicker label="Startdatum" value={startDate} onChange={setStartDate}/></label><label>Übergabestatus<ScheduleSelect label="Übergabestatus" value={handover} options={["Übergabe vollständig gelesen", "Übergabe teilweise gelesen", "Übergabe ausstehend"]} onChange={setHandover}/></label><fieldset className="area-editor-wide duty-assignment-options"><legend>Start-Checkliste</legend><div className="area-service-options"><label className={checklist.includes("Übergabe") ? "selected" : ""}><input type="checkbox" checked={checklist.includes("Übergabe")} onChange={() => toggleChecklist("Übergabe")}/><span>Übergabe geprüft</span></label><label className={checklist.includes("Medikationswagen") ? "selected" : ""}><input type="checkbox" checked={checklist.includes("Medikationswagen")} onChange={() => toggleChecklist("Medikationswagen")}/><span>Medikationswagen übernommen</span></label><label className={checklist.includes("Notfalltelefon") ? "selected" : ""}><input type="checkbox" checked={checklist.includes("Notfalltelefon")} onChange={() => toggleChecklist("Notfalltelefon")}/><span>Notfalltelefon geprüft</span></label></div></fieldset><label className="area-editor-wide">Notiz zum Dienststart<textarea placeholder="z. B. besondere Prioritäten für den Frühdienst …" rows={5}/></label></div><div className="duty-assignment-summary"><span><strong>{shift}</strong><small>{unit} · Anna Meier</small></span><span><strong>{formatScheduleDate(startDate)}</strong><small>{checklist.length}/3 Punkte geprüft · {handover}</small></span></div><footer className="area-editor-actions"><button className="secondary-button" type="button" onClick={onClose}>Abbrechen</button><button className="primary-button" type="submit"><ModuleIcon name="check"/> Dienst starten</button></footer></form></section></div>;
}
