        ...
        <div className={`schedule-date-menu ${openUp ? "up" : ""}`} role="dialog" aria-label={`${label} auswählen`}>
          ...
          <div className="schedule-date-menu-footer">
            ...
            <button
              type="button"
              onClick={() => {
                setViewMonth(new Date("2026-09-14T12:00:00"));
                onChange("2026-09-14");
                setOpen(false);
              }}
            >
    ...
    try {
      const response = await fetch("/api/tasks", {
        ...
        body: JSON.stringify({
          ...
          priority: priority === "Kritisch" ? "critical" : priority === "Dringend" ? "high" : "normal",
          residentName: resident.split(" · ")[0],
          ownerName: owner,
      ...
      >
        <header className="area-editor-header">
          <div>
            ...
            <h2 id="task-editor-title">Aufgabe erstellen</h2>
        ...
        <form className="area-editor-form" onSubmit={createTask}>
          <div className="area-editor-intro">
            <span className="area-editor-icon">
              <ModuleIcon name="tasks" />
            </span>
            <div>
              <strong>Neue Teamaufgabe</strong>
              <p>Die Aufgabe erscheint im persönlichen und im gemeinsamen Arbeitsbereich.</p>
            </div>
            <span className="duty-assignment-status">
              <i />
              Entwurf
          ...
          <div className="area-editor-grid">
            ...
            <label>
              ...
              <ScheduleSelect
                label="Kategorie"
                value={category}
                options={["Pflege", "Medikation", "Vitalwerte", "Dokumentation", "Wundmanagement", "Ernährung"]}
            ...
            <label>
              ...
              <ScheduleSelect
                label="Priorität"
                value={priority}
                options={["Normal", "Dringend", "Kritisch"]}
                onChange={setPriority}
              />
            </label>
            <label className="area-editor-wide">
            ...
            <label>
              ...
              <ScheduleSelect
                label="Fällig um"
                value={dueTime}
                options={["08:00", "09:00", "10:00", "11:30", "14:00", "16:00"]}
                onChange={setDueTime}
              />
            </label>
            <label>
            ...
            <label>
              ...
              <ScheduleSelect
                label="Wiederholung"
                value={repeat}
                options={["Einmalig", "Täglich", "Wöchentlich", "Individuell"]}
                onChange={setRepeat}
              />
            </label>
            <label className="area-editor-wide">
          ...
          <footer className="area-editor-actions">
            ...
            <button className="primary-button" type="submit" disabled={saving}>
              <ModuleIcon name="check" /> {saving ? "Speichern…" : "Aufgabe erstellen"}
            </button>
          </footer>
...
function ShiftStartEditor({
  ...
  const [unit, setUnit] = useState("Wohnbereich 2");
  const [shift, setShift] = useState("Frühdienst");
  const [handover, setHandover] = useState("Übergabe vollständig gelesen");
  const [startDate, setStartDate] = useState("2026-09-15");
  const [checklist, setChecklist] = useState(["Übergabe", "Medikationswagen"]);
      ...
      >
        ...
        <form
          ...
          onSubmit={(event) => {
            event.preventDefault();
            onClose();
            showToast(`${shift} im ${unit} wurde gestartet`);
          }}
        >
          <div className="area-editor-intro">
            <span className="area-editor-icon">
          ...
          <div className="area-editor-grid">
            <label>
              ...
              <ScheduleSelect
                ...
                value={unit}
                options={["Wohnbereich 1", "Wohnbereich 2", "Wohnbereich 3", "Gesamtes Haus"]}
                onChange={setUnit}
              />
            </label>
            <label>
            ...
            <label>
              ...
              <ScheduleSelect
                ...
                value={handover}
                options={["Übergabe vollständig gelesen", "Übergabe teilweise gelesen", "Übergabe ausstehend"]}
            ...
            <fieldset className="area-editor-wide duty-assignment-options">
              ...
              <div className="area-service-options">
                <label className={checklist.includes("Übergabe") ? "selected" : ""}>
                  <input
                    type="checkbox"
                    checked={checklist.includes("Übergabe")}
                    onChange={() => toggleChecklist("Übergabe")}
                  />
                  <span>Übergabe geprüft</span>
                </label>
                <label className={checklist.includes("Medikationswagen") ? "selected" : ""}>
                  <input
                    type="checkbox"
                    checked={checklist.includes("Medikationswagen")}
                    onChange={() => toggleChecklist("Medikationswagen")}
                  />
                  <span>Medikationswagen übernommen</span>
                </label>
                <label className={checklist.includes("Notfalltelefon") ? "selected" : ""}>
                  <input
                    type="checkbox"
                    checked={checklist.includes("Notfalltelefon")}
                    onChange={() => toggleChecklist("Notfalltelefon")}
                  />
                  <span>Notfalltelefon geprüft</span>
                </label>
              </div>
            </fieldset>
            <label className="area-editor-wide">
              Notiz zum Dienststart
              <textarea placeholder="z. B. besondere Prioritäten für den Frühdienst …" rows={5} />
            </label>
          </div>
          <div className="duty-assignment-summary">
            <span>
              <strong>{shift}</strong>
              <small>{unit} · Anna Meier</small>
            </span>
            <span>
              <strong>{formatScheduleDate(startDate)}</strong>
              <small>
                {checklist.length}/3 Punkte geprüft · {handover}
              </small>
            </span>
          </div>
          <footer className="area-editor-actions">
            ...
            <button className="primary-button" type="submit">
              <ModuleIcon name="check" /> Dienst starten
            </button>
          </footer>
        </form>
      </section>
...
> = {
  shift: {
    ...
    title: "Mein Dienst",
    description: "Dein persönlicher Schichtarbeitsplatz für heute.",
    action: "Dienst starten",
  },
  shiftHistory: {
    module: "shift",
    child: "Schichtverlauf",
    eyebrow: "CareCore Shift",
    title: "Schichtverlauf",
    description: "Abgeschlossene Dienste, Übergaben und dokumentierte Aktivitäten.",
  ...
  tasks: {
    ...
    description: "Alle offenen Aufgaben und Interventionen für deinen Dienst.",
...
const shiftTasks: Task[] = [
  {
    ...
    detail: "Wohnbereich 2 · Pflegezimmer",
    category: "Übergabe",
    tone: "info",
  },
  {
  ...
  },
  {
    ...
    detail: "Frau Schneider · Zimmer 104",
    category: "Gespräch",
    tone: "info",
  },
];
...
const shiftHistory = [
  {
    date: "Freitag, 11. September 2026",
    time: "07:00–15:30",
    unit: "Wohnbereich 2",
  ...
  {
    date: "Donnerstag, 10. September 2026",
    time: "13:30–22:00",
    unit: "Wohnbereich 2",
    handover: "Vollständig",
    entries: 24,
  ...
  {
    date: "Mittwoch, 9. September 2026",
    time: "07:00–15:30",
    unit: "Wohnbereich 2",
    handover: "2 Hinweise offen",
    entries: 16,
    tone: "attention" as Tone,
  },
  {
    date: "Dienstag, 8. September 2026",
    time: "07:00–15:30",
    unit: "Wohnbereich 1",
    handover: "Vollständig",
    entries: 21,
    ...
    <section className="operations-pulse" aria-label="Operativer Puls">
      <div className="operations-pulse-intro">
        ...
        <div>
          ...
          <strong>Frühdienst · 07:00–15:30</strong>
          <small>Live-Überblick für deinen Arbeitsbereich</small>
        </div>
      </div>
      <button type="button" onClick={() => showToast("Aktualisierung ist bereits aktiv")}>
        <span>
          <strong>12</strong>
...
function ShiftView({ showToast }: { showToast: (message: string) => void }) {
  ...
  return (
    <>
      <Summary
        items={[
          { icon: "calendar", value: "07:00–15:30", label: "Frühdienst" },
          { icon: "residents", value: "12", label: "Bewohner auf Station" },
          { icon: "tasks", value: "5", label: "Aufgaben heute", tone: "attention" },
          { icon: "check", value: `${done.length}/5`, label: "Dienstfortschritt", tone: "info" },
        ]}
      />
      <div className="operations-grid">
        <section className="card operations-timeline">
          <div className="card-header">
            <div>
              <p className="eyebrow">Montag, 14. September 2026</p>
              <h2 className="card-title">Dein heutiger Dienst</h2>
              <p className="card-subtitle">Wohnbereich 2 · Pflegezimmer</p>
            </div>
            <button
              className="secondary-button"
              type="button"
              onClick={() => showToast("Ansicht auf den ganzen Tag erweitert")}
            >
              Ganzer Tag
            </button>
          </div>
          <div>
            {shiftTasks.map((task) => (
              <article className={`operations-timeline-row ${done.includes(task.id) ? "complete" : ""}`} key={task.id}>
                <time>{task.time}</time>
                <span className={`operations-timeline-icon ${task.tone}`}>
                  <ModuleIcon
                    name={
                      task.category === "Medikation"
                        ? "med"
                        : task.category === "Wunde"
                          ? "wounds"
                          : task.category === "Übergabe"
                            ? "handover"
                            : task.category === "Gespräch"
                              ? "team"
                              : "note"
                    }
                  />
                ...
                <button
                  type="button"
                  aria-label={`${task.title} ${done.includes(task.id) ? "rückgängig" : "erledigt markieren"}`}
                  onClick={() =>
                    setDone((current) =>
                ...
                >
                  <ModuleIcon name={done.includes(task.id) ? "check" : "plus"} />
                </button>
              </article>
            ))}
          </div>
        ...
        <aside className="operations-sidebar">
          <section className="card">
            ...
            <div className="operations-note-list">
              <p className="critical">
                ...
                <span>
                  <strong>Sturzrisiko · Hans Müller</strong>
                  <small>Neurologische Kontrolle bis 14:00 Uhr nach Standard durchführen.</small>
                </span>
              </p>
              <p className="attention">
                <ModuleIcon name="med" />
                <span>
                  <strong>Dosisänderung · Erika Meier</strong>
                  <small>Metoprolol ab heute 50 mg. Wirkung beobachten.</small>
                </span>
              </p>
              <p className="info">
                <ModuleIcon name="note" />
                <span>
                  <strong>Neue Dokumentation</strong>
                  <small>2 Einträge warten auf deine Prüfung.</small>
                </span>
              </p>
            </div>
          </section>
          <section className="card operations-quick">
            ...
            <div>
              <button type="button" onClick={() => showToast("Dokumentation vorbereitet")}>
                <ModuleIcon name="note" />
              ...
              <button type="button" onClick={() => showToast("Neue Aufgabe vorbereitet")}>
                <ModuleIcon name="tasks" />
                <span>Aufgabe erstellen</span>
              </button>
              <button type="button" onClick={() => showToast("Übergabe geöffnet")}>
...
function ShiftHistoryView({ showToast }: { showToast: (message: string) => void }) {
  ...
  const filtered = shiftHistory.filter((entry) =>
    `${entry.date} ${entry.unit} ${entry.handover}`
  ...
  return (
    <>
      <Summary
        items={[
          { icon: "calendar", value: "24", label: "Dienste im Quartal" },
          { icon: "check", value: "22", label: "Übergaben vollständig" },
          { icon: "note", value: "99", label: "Dokumentationen", tone: "info" },
          { icon: "alert", value: "2", label: "Hinweise offen", tone: "attention" },
        ]}
      />
        ...
        <div className="operations-toolbar">
          <div>
            ...
            <p className="card-subtitle">
              {filtered.length} von {shiftHistory.length} Einträgen
            </p>
          </div>
          <label className="resident-search">
            ...
            <input
              ...
              placeholder="Datum oder Wohnbereich"
          ...
          <button
            className="secondary-button"
            type="button"
            onClick={() => showToast("Schichtbericht wird vorbereitet")}
          >
            Exportieren
          </button>
        </div>
          ...
          {filtered.map((entry) => (
            <article key={entry.date}>
              <span className="operations-date">
                ...
                <small>{entry.date.split(", ").slice(1).join(", ")}</small>
              </span>
              <span>
              ...
              <span>
                <strong>{entry.entries} Dokumentationen</strong>
                <small>Übergabe: {entry.handover}</small>
              </span>
              <span className={`status-badge ${entry.tone}`}>
                {entry.handover === "Vollständig" ? "Abgeschlossen" : "Prüfung nötig"}
              </span>
              <button type="button" onClick={() => showToast(`${entry.date} geöffnet`)}>
                Ansehen <ModuleIcon name="chevron" />
              </button>
            </article>
          ))}
        </div>
...
function TaskView({ team, showToast }: { team: boolean; showToast: (message: string) => void }) {
  ...
  useEffect(() => {
    ...
    return () => {
      active = false;
      window.removeEventListener("carecore:tasks-changed", load);
    };
  }, []);
  ...
  const filtered = items.filter((row) => {
    ...
    if (filter === "Überfällig")
      return row.status !== "completed" && Boolean(row.due_at && new Date(row.due_at).getTime() < now);
  ...
  const filters = team
    ...
    : ["Alle", "Offen", "Heute fällig", "Überfällig"];
  async function toggleTask(row: TaskRow) {
    ...
    try {
      const response = await fetch("/api/tasks", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id: row.id, completed }),
      });
      if (!response.ok) throw new Error("Aufgabe konnte nicht aktualisiert werden.");
      setRows((current) =>
        current.map((item) => (item.id === row.id ? { ...item, status: completed ? "completed" : "open" } : item)),
      );
      ...
      <Summary
        items={[
          { icon: "tasks", value: String(items.length), label: team ? "Aufgaben im Team" : "Aufgaben für dich" },
          {
            icon: "alert",
            value: String(
              items.filter((row) => ["high", "critical"].includes(row.priority) && row.status !== "completed").length,
            ),
            label: "dringend",
            tone: "critical",
          },
          {
            icon: "calendar",
            value: String(items.filter((row) => row.due_at && new Date(row.due_at).toDateString() === today).length),
            label: "heute fällig",
            tone: "attention",
          },
          {
            icon: "check",
            value: String(items.filter((row) => row.status === "completed").length),
            label: "erledigt",
            tone: "info",
          },
        ]}
      ...
      <section className="card operations-list-card">
        <div className="operations-toolbar">
          <div>
            ...
            <p className="card-subtitle">{filtered.length} Aufgaben sichtbar</p>
          </div>
        ...
        <div className="task-list">
          {filtered.map((task) => {
            ...
            return (
              <article className={`task-row ${done ? "complete" : ""}`} key={task.id}>
                <button
                  className="task-check"
                  type="button"
                  aria-label={`${task.title} ${done ? "rückgängig" : "erledigt markieren"}`}
                  onClick={() => void toggleTask(task)}
                >
                  <ModuleIcon name={done ? "check" : "plus"} />
                </button>
                ...
                <div>
                  <strong>{task.title}</strong>
                  <small>{[task.resident_name, task.description].filter(Boolean).join(" · ")}</small>
                </div>
                {team && (
                ...
                <span className={`status-badge ${done ? "stable" : tone}`}>
                  {done
                    ? "Erledigt"
                ...
                <button
                  ...
                  type="button"
                  onClick={() => showToast(task.description || task.title)}
                >
                  Details
                </button>
              </article>
...
export default function OperationsWorkspace({ view }: { view: OperationsView }) {
  ...
  const [shiftStartEditorOpen, setShiftStartEditorOpen] = useState(false);
  return (
    <ModulePageShell
        ...
        <main className="workspace module-workspace operations-command-workspace">
          <section className="page-heading care-page-heading" aria-labelledby="operations-title">
            <div className="heading-copy">
              ...
              <h1 id="operations-title">{meta.title}</h1>
              ...
              onClick={() =>
                view === "schedule"
                  ...
                  : view === "teamSchedule"
                    ...
                    : view === "tasks" || view === "teamTasks"
                      ? setTaskEditorOpen(true)
                      : view === "shift"
                        ? setShiftStartEditorOpen(true)
                        : showToast(`${meta.action} vorbereitet`)
              }
            >
          ...
          <TaskEditor open={taskEditorOpen} onClose={() => setTaskEditorOpen(false)} showToast={showToast} />
          <ShiftStartEditor
            open={shiftStartEditorOpen}
            onClose={() => setShiftStartEditorOpen(false)}
            showToast={showToast}
          />
          <OperationsPulse showToast={showToast} />
          {view === "shift" && <ShiftView showToast={showToast} />}{" "}
