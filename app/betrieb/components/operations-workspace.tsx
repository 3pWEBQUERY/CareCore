...
function ScheduleView({ team, showToast }: { team: boolean; showToast: (message: string) => void }) {
  ...
  return <><Summary items={[{ icon: "calendar", value: team ? "32" : "5", label: team ? "Dienste geplant" : "Dienste diese Woche" }, { icon: "residents", value: team ? "92%" : "36 h", label: team ? "Besetzung" : "Arbeitszeit", tone: "info" }, { icon: "alert", value: team ? "3" : "1", label: team ? "offene Dienste" : "Änderungsanfragen", tone: "attention" }, { icon: "check", value: team ? "18" : "0", label: team ? "Bestätigt" : "Ferientage" }]}/><section className="card schedule-card"><div className="card-header"><div><p className="eyebrow">September 2026 · KW 38</p><h2 className="card-title">{team ? "Teamplanung" : "Deine Woche"}</h2></div><button className="secondary-button" type="button" onClick={() => showToast("Kalenderansicht wird vorbereitet")}>Heute</button></div><div className="schedule-week">{days.map((day, index) => <button className={selectedDay === index ? "active" : ""} type="button" key={day.date} onClick={() => setSelectedDay(index)}><strong>{day.day}</strong><span>{day.date}</span><i className={day.shifts[0] || day.shifts[1] ? "has-shift" : ""}/></button>)}</div><div className="schedule-day-detail"><div><p className="eyebrow">{days[selectedDay].day}, {days[selectedDay].date}</p><h3>{team ? "Besetzung im Wohnbereich" : "Deine Einsätze"}</h3><p>{team ? "Wohnbereich 2 · Früh- und Spätdienst" : "Wohnbereich 2 · 1. OG"}</p></div>{team ? <div className="schedule-team-grid">{teamMembers.slice(0, 4).map((member) => <button type="button" key={member.initials} onClick={() => setSelectedMember(member.initials)} className={selectedMember === member.initials ? "selected" : ""}><span className="avatar">{member.initials}</span><span><strong>{member.name}</strong><small>{member.shifts[selectedDay] || "Frei"}</small></span></button>)}</div> : <div className="schedule-shift-detail"><span className="schedule-shift-icon"><ModuleIcon name="shift"/></span><span><strong>{days[selectedDay].shifts[0] || "Frei"}</strong><small>{days[selectedDay].shifts[0] ? "Frühdienst · Pflegefachfrau HF" : "Keine Einteilung geplant"}</small></span><span className="status-badge stable">Bestätigt</span></div>}</div></section></>;
}
