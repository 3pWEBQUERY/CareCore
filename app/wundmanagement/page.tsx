...
const wounds: WoundCase[] = [
  {
    ...
    status: "Kritisch",
    tone: "critical",
    lastCare: "Heute, 07:55",
    nextCare: "Morgen, 08:00",
    progress: 32,
    owner: "Lea Frei",
  },
  {
    ...
    tone: "attention",
    lastCare: "Heute, 06:50",
    nextCare: "Heute, 14:00",
    progress: 54,
    owner: "Anna Meier",
  },
  {
    ...
    status: "In Behandlung",
    tone: "attention",
    lastCare: "Gestern, 17:20",
    nextCare: "Heute, 10:30",
    progress: 61,
    owner: "Nora Baumann",
  },
  {
    ...
    status: "Heilend",
    tone: "stable",
    lastCare: "Gestern, 15:40",
    nextCare: "Morgen, 09:15",
    progress: 82,
    owner: "Lea Frei",
  },
  {
    ...
    status: "Heilend",
    tone: "stable",
    lastCare: "8. September",
    nextCare: "Heute, 11:15",
    progress: 74,
    owner: "Anna Meier",
  },
];
...
export default function WoundOverviewPage() {
  ...
  const mobileModules = mobileGroup
    ? [
        ...mobileGroup.modules.filter((module) => module.id === "wounds"),
        ...mobileGroup.modules.filter((module) => module.id !== "wounds"),
      ].slice(0, 3)
    : [];
  ...
  const filteredWounds = useMemo(
    () =>
      wounds.filter((wound) => {
        const searchable = `${wound.resident} ${wound.room} ${wound.location} ${wound.diagnosis}`.toLocaleLowerCase(
          "de-CH",
        );
        return (
          (status === "Alle" || wound.status === status) && searchable.includes(query.trim().toLocaleLowerCase("de-CH"))
        );
      }),
    [query, status],
  );
  ...
  return (
    <div className="app-shell wounds-page">
      <AppSidebar activeModule="wounds" activeChild="Wundübersicht" onToast={setToast} />
        ...
        <main className="workspace wounds-workspace">
          <section className="page-heading wounds-heading" aria-labelledby="wounds-page-title">
            <div className="heading-copy">
              ...
              <h1 id="wounds-page-title">Wundübersicht</h1>
              <p>Aktive Wunden, anstehende Versorgungen und Heilungsverläufe im Blick.</p>
            </div>
            <button
              className="primary-button"
              type="button"
              onClick={() => setToast("Neue Wunddokumentation vorbereitet")}
            >
          ...
          <section className="wound-summary" aria-label="Wundstatus">
            ...
            <div>
              ...
              <span>
                <strong>3</strong>
                <small>Versorgungen heute</small>
              </span>
            </div>
            <div>
              ...
              <span>
                <strong>1</strong>
                <small>kritischer Verlauf</small>
              </span>
            </div>
            <div>
              <span className="summary-icon">
              ...
              <span>
                <strong>68%</strong>
                <small>mittlerer Heilungsfortschritt</small>
              </span>
            </div>
          </section>

          <section className="critical-alert wound-alert" aria-label="Dringende Wundversorgung">
            <span className="critical-symbol">
              <Icon name="alert" />
            </span>
            <div>
              <strong>Heute priorisieren · Maria Keller</strong>
              <p>Dekubitus im Sakralbereich: Verbandkontrolle und Fotodokumentation bis 14:00 Uhr.</p>
            </div>
            <button className="secondary-button" type="button" onClick={() => setSelectedWoundId("w2")}>
              Fall auswählen <Icon name="chevron" className="button-icon" />
            </button>
          </section>
          ...
          <div className="wound-overview-layout">
            <section className="card wound-directory" aria-labelledby="wound-directory-title">
              <div className="wound-toolbar">
                <div>
                  ...
                  <p className="card-subtitle">
                    {filteredWounds.length} von {wounds.length} Wundfällen
                  </p>
                </div>
                ...
                <div className="wound-status-filter" aria-label="Wundstatus filtern">
                  {(["Alle", "Kritisch", "In Behandlung", "Heilend"] as WoundStatus[]).map((filter) => (
                    <button
                ...
                <button
                  ...
                  type="button"
                  onClick={() => setToast("Weitere Wundfilter geöffnet")}
                >
                  <Icon name="filter" />
                  Filter
                </button>
              ...
              <div className="wound-case-list">
                {filteredWounds.map((wound) => (
                  <button
                    className={`wound-case-row ${selectedWound.id === wound.id ? "selected" : ""}`}
                    type="button"
                    key={wound.id}
                    onClick={() => setSelectedWoundId(wound.id)}
                  >
                    <span className={`resident-avatar ${wound.tone === "critical" ? "critical" : ""}`}>
                      {wound.initials}
                    </span>
                    <span className="wound-case-main">
                      <strong>{wound.resident}</strong>
                      <small>
                        {wound.room} · {wound.location}
                      </small>
                      <span>
                        {wound.diagnosis} · {wound.size}
                      </span>
                    </span>
                    <span className="wound-care-date">
                      <strong>{wound.lastCare}</strong>
                      <small>{wound.owner}</small>
                    </span>
                    <span className="wound-progress">
                      <span>
                        <i style={{ width: `${wound.progress}%` } as CSSProperties} />
                      </span>
                      <strong>{wound.progress}%</strong>
                    </span>
                ...
                {filteredWounds.length === 0 && (
                  <div className="resident-empty">
                    <Icon name="search" />
                    <strong>Keine Wundfälle gefunden</strong>
            ...
            <aside className="wound-sidebar">
              <section className="card wound-focus-card" aria-live="polite">
                <div className="card-header">
                  <div>
                    ...
                    <h2 className="card-title">{selectedWound.resident}</h2>
                    <p className="card-subtitle">{selectedWound.room}</p>
                  </div>
                  <span className={`status-badge ${selectedWound.tone}`}>{selectedWound.status}</span>
                </div>
                <div className="wound-focus-body">
                  <span className={`wound-focus-icon ${selectedWound.tone}`}>
                    <Icon name="wounds" />
                  </span>
                  <h3>{selectedWound.location}</h3>
                  <p>
                    {selectedWound.diagnosis} · {selectedWound.size}
                  </p>
                  <dl>
                    <div>
                      <dt>Letzte Versorgung</dt>
                      <dd>{selectedWound.lastCare}</dd>
                    </div>
                    <div>
                      <dt>Nächste Versorgung</dt>
                      <dd>{selectedWound.nextCare}</dd>
                    </div>
                    <div>
                      <dt>Verantwortlich</dt>
                      <dd>{selectedWound.owner}</dd>
                    </div>
                  </dl>
                  <div className="wound-focus-progress">
                    <span>
                      <i style={{ width: `${selectedWound.progress}%` } as CSSProperties} />
                    </span>
                    <small>{selectedWound.progress}% Heilungsfortschritt</small>
                  </div>
                  <div className="wound-focus-actions">
                    <button
                      className="primary-button"
                      type="button"
                      onClick={() => setToast(`Wundakte von ${selectedWound.resident} geöffnet`)}
                    >
                      Wundakte öffnen
                    </button>
                    <button
                      className="secondary-button"
                      type="button"
                      onClick={() => setToast("Neue Verlaufskontrolle vorbereitet")}
                    >
                      Verlauf erfassen
                    </button>
                  </div>
              ...
              <section className="card wound-schedule-card">
                <div className="card-header">
                  <div>
                    <h2 className="card-title">Heute zu versorgen</h2>
                    <p className="card-subtitle">3 geplante Maßnahmen</p>
                  </div>
                </div>
                <div className="wound-schedule-list">
                  <button type="button" onClick={() => setSelectedWoundId("w2")}>
                    <time>14:00</time>
                    <span>
                      <strong>Maria Keller</strong>
                      <small>Verbandkontrolle · Sakralbereich</small>
                    </span>
                    <span className="status-badge critical">Priorität</span>
                  </button>
                  <button type="button" onClick={() => setSelectedWoundId("w3")}>
                    <time>10:30</time>
                    <span>
                      <strong>Erika Meier</strong>
                      <small>Kompressionsverband · Unterschenkel</small>
                    </span>
                  </button>
                  <button type="button" onClick={() => setSelectedWoundId("w5")}>
                    <time>11:15</time>
                    <span>
                      <strong>Peter Aebischer</strong>
                      <small>Wundkontrolle · Handrücken</small>
                    </span>
                  </button>
                </div>
              </section>
              ...
              {navigation.map((group) => (
                <button type="button" key={group.id} onClick={() => chooseMobileGroup(group.id)}>
                  <span className="mobile-nav-group-icon">
                    <Icon name={group.modules[0]?.icon ?? "pulse"} />
                  </span>
                  <span>
      ...
      <nav className="bottom-nav" aria-label="Mobile Navigation">
        ...
        {mobileModules.slice(0, 4).map((module) => {
          ...
          return (
            <button
              className={module.id === "wounds" ? "active" : ""}
              type="button"
              key={module.id}
          ...
          <section className="search-dialog" role="dialog" aria-modal="true" aria-label="Globale Suche">
            <div className="search-input-wrap">
              ...
              <input
                ...
                value={query}
                onChange={(event) => setQuery(event.target.value)}
                placeholder="Bewohner oder Wundfall suchen…"
            ...
            <div className="search-results">
              <span className="search-group-label">Wundfälle</span>
              {filteredWounds.map((wound) => (
                <button
                ...
                >
                  ...
                  <span>
                    <strong>
                      {wound.resident} · {wound.location}
                    </strong>
                    <small>
                      {wound.room} · {wound.diagnosis}
                    </small>
                  </span>
                </button>
              ))}
