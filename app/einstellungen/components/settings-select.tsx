...
export default function SettingsSelect({ label, value, options }: { label: string; value?: string; options?: string[] }) {
  ...
  return <div className="settings-custom-select" ref={rootRef}><button className="settings-select-trigger" type="button" aria-haspopup="listbox" aria-expanded={open} aria-label={label} onClick={toggleOpen}><span>{selected}</span><ModuleIcon name="caretDown" className={open ? "open" : ""}/></button>{open && <div className={`settings-select-menu ${openUp ? "up" : ""}`} role="listbox" aria-label={label}>{(options ?? []).map((option) => <button className={selected === option ? "selected" : ""} type="button" role="option" aria-selected={selected === option} key={option} onClick={() => { setSelected(option); setOpen(false); setOpenUp(false); }}>{option}<ModuleIcon name="check"/></button>)}</div>}</div>;
}
