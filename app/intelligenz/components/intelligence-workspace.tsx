...
export default function IntelligenceWorkspace({ view }: { view: IntelligenceView }) {
  ...
  const [selected, setSelected] = useState("Übergabe zusammenfassen");
  const prompts = ["Übergabe zusammenfassen", "Risiken für die Visite prüfen", "Dokumentation vorbereiten"];
  return (
    <ModulePageShell
      activeModule="ai"
      ...
      {(showToast) => (
        <main className="workspace intelligence-workspace">
          <section className="intelligence-heading page-heading">
            ...
            <span className="intelligence-trust">
              <ModuleIcon name="check" /> Datenschutz geprüft
            </span>
          </section>
          {view === "assistant" ? (
              ...
              <section className="card intelligence-chat">
                <div className="card-header">
                  <div>
                    ...
                    <p className="card-subtitle">Die Antwort bleibt im Kontext deiner aktuellen Arbeitsfläche.</p>
                  </div>
                ...
                <div className="intelligence-suggestions">
                  {prompts.map((item) => (
                    ...
                    >
                      <ModuleIcon
                        name={item.includes("Risiken") ? "alert" : item.includes("Dokumentation") ? "note" : "handover"}
                      />
                      {item}
                    </button>
                  ))}
                </div>
                <div className="intelligence-conversation">
                  <div className="intelligence-message assistant">
                    <span className="intelligence-avatar">
                      <ModuleIcon name="ai" />
                    </span>
                    <p>
                      Hallo Anna! Ich kann Übergaben strukturieren, Risiken hervorheben und Dokumentationsentwürfe
                      vorbereiten. Die finale Freigabe bleibt immer bei dir.
                    </p>
                  </div>
                  {submitted && (
                ...
                <form
                  ...
                  onSubmit={(event) => {
                    ...
                    setSubmitted(true);
                    showToast("Assistenzantwort vorbereitet");
                  }}
                >
                  <input
                    value={prompt}
              ...
              <aside className="intelligence-side">
                <section className="card">
                  ...
                  <div className="intelligence-context">
                    ...
                    <span>12 Bewohner · Frühdienst</span>
                    <span>5 offene Aufgaben</span>
                    <span>2 Hinweise zur Prüfung</span>
                  </div>
                </section>
                <section className="card">
                  <div className="card-header">
                  ...
                  <p className="intelligence-note">
                    KI-Vorschläge sind Entwürfe und werden erst nach deiner Prüfung in CareCore gespeichert.
                  </p>
                </section>
              </aside>
            </div>
              ...
              <section className="card intelligence-drafts">
                <div className="operations-toolbar">
                  <div>
                    ...
                    <p className="card-subtitle">3 Vorschläge warten auf deine Entscheidung</p>
                  </div>
                  <button className="secondary-button" type="button" onClick={() => showToast("Entwürfe aktualisiert")}>
                    Aktualisieren
                  </button>
                </div>
                <div className="intelligence-draft-list">
                  {[
                    ["Übergabe · Hans Müller", "Sturzrisiko und Kontrollbedarf strukturiert", "Heute, 08:10", "Prüfen"],
                    ["Verlaufsnotiz · Maria Keller", "Blutzuckerverlauf mit Visithinweis", "Gestern, 16:20", "Prüfen"],
                    ["Pflegeplanung · Erika Meier", "Ziel zur Mobilität vorgeschlagen", "11.09.2026", "Freigabe"],
                  ].map(([title, detail, time, action]) => (
                    <article key={title}>
                      ...
                      <span>
                        <strong>{title}</strong>
                        <small>{detail}</small>
                        <em>{time} · CareCore KI</em>
                      </span>
                      <button className="quiet-button" type="button" onClick={() => showToast(`${title} geöffnet`)}>
                        {action}
                      </button>
                    </article>
                  ))}
