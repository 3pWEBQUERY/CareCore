...
export type WorkContext = {
  profile: { displayName: string; jobTitle: string; phone: string; primaryCareUnitId: string | null; primaryCareUnitName: string | null; organizationName: string };
  careUnits: CareUnit[];
...
async function seedDemoContext(userId: string) {
  ...
  await sql`INSERT INTO carecore_user_profiles (user_id, organization_id, job_title, phone, primary_care_unit_id) VALUES (${userId}, ${organizationId}, 'Pflegefachfrau HF', '+41 79 555 12 34', ${unitRows[1][0]}) ON CONFLICT (user_id) DO UPDATE SET organization_id = COALESCE(carecore_user_profiles.organization_id, EXCLUDED.organization_id), job_title = COALESCE(NULLIF(carecore_user_profiles.job_title, ''), EXCLUDED.job_title), phone = COALESCE(NULLIF(carecore_user_profiles.phone, ''), EXCLUDED.phone), primary_care_unit_id = COALESCE(carecore_user_profiles.primary_care_unit_id, EXCLUDED.primary_care_unit_id), updated_at = NOW()`;
...
export async function getWorkContext(userId: string): Promise<WorkContext> {
  ...
  return {
    profile: { displayName: profile.display_name, jobTitle: profile.job_title, phone: profile.phone, primaryCareUnitId: profile.primary_care_unit_id, primaryCareUnitName: profile.primary_care_unit_name, organizationName: profile.organization_name },
